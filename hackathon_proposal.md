...
![Urja-85 Logo]("D:\HACKATHONS AND BUILDATHONS\INDEPENDECE DAY PROJECT\urja_85_logo_.jpg")
...
```mermaid
graph TD
    A[Mobile App / PWA] -->|tRPC / WebSockets| B(Next.js API Gateway)
    
    subgraph AI Processing
    B --> C{Python FastAPI}
    C -->|OCR/Vision| D[RC Verification]
    C -->|Predictive AI| E[Route Planner]
    end
    
    subgraph Government DPI
    B --> F[Bhashini API Voice]
    B --> G[DigiLocker Auth]
    B --> H[UPI Payments API]
    end
    
    subgraph Data & Web3
    B --> I[(Supabase / Postgres)]
    B --> J[(Redis Cache)]
    B --> K[Polygon Testnet Smart Contracts]
    K --> L[Carbon Credit Ledger]
    end
```
